...
public class CrossHair : MonoBehaviour
{
    ...
    [SerializeField] Vector2 camBoundery;
    ...
    void CameraLimit()
    {
        if (tf_Cam.localPosition.x >= camBoundery.x) 
        {
            tf_Cam.localPosition = new Vector3(camBoundery.x, tf_Cam.localPosition.y, tf_Cam.localPosition.z);
        }
        else if (tf_Cam.localPosition.x <= -camBoundery.x)
        {
            tf_Cam.localPosition = new Vector3(-camBoundery.x, tf_Cam.localPosition.y, tf_Cam.localPosition.z);
        }
    }
